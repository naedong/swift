//
//  BackGroundDetailEditorVIew.swift
//  myprojec
//
...
struct BackGroundControllVIew: View {
    @Binding var colors : ColorsModel
    ...
    var body: some View {
        GeometryReader { geom in
            ...
            VStack(spacing: 0) {
                if drawGradient {
                    LinearGradient(gradient: colors.gradient, startPoint: .leading, endPoint: .trailing)
                        .border(.secondary, width: 1)
                }
                ZStack {
                    ForEach(colors.stops) { stop in
                        StopHandle(stop: stop, gradient: $colors, selectedStopID: $selectedStopID, width: geom.size.width)
