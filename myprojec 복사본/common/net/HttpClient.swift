...
class HttpClient<T: Decodable>: ObservableObject {
    ...
    func checkWeb(session : String) -> HTTPMethod {
        if session.contains("login/login")
        {
            return .post
        }
        else if session.contains("login/sendtwo"){
            return .post
        }else if session.contains("login/sendone"){
            return .post
        }else if session.contains("login/signs"){
            return .post
        }
        else if session.contains("user/setting"){
            return .post
        }
        else {
            return.get
    ...
    //    }
        ...
        func alamofireNetworkingLogin(url: String, onSuccess: @escaping onSuccess, onFailure: @escaping onFailure, param :LoginModel) {
            ...
            AF.request(sessionUrl,
                       method: checkWeb(session: sessionUrl.absoluteString),
                       parameters: param,
            ...
            func alamofireNetworking(url: String, onSuccess: @escaping onSuccess, onFailure: @escaping onFailure) {
                ...
                .responseDecodable(of: T.self){ response in
                    ...
                    case .failure(let error):
                        print(error)
                        if let validata = response.data, validata.count > 4{
                            
                        }
                        
                        //                if let validata = response.data, validata.count > 0 {
    ...
    func alamofireNetworkingPhone(url: String, onSuccess: @escaping onSuccess, onFailure: @escaping onFailure, param :PhoneModel) {
        print("alamofireNetworking")
        guard let sessionUrl = URL(string: url) else {
            print("Invalid URL")
            return
    ...
    func alamofireNetworkingPhoneVertification(url: String, onSuccess: @escaping onSuccess, onFailure: @escaping onFailure, param :PhoneModel) {
        print("alamofireNetworking")
        guard let sessionUrl = URL(string: url) else {
            print("Invalid URL")
            return
    ...
    func alamofireNetworkingSignUp(url: String, onSuccess: @escaping onSuccess, onFailure: @escaping onFailure, param : SignModel) {
        print("alamofireNetworking")
        guard let sessionUrl = URL(string: url) else {
            print("Invalid URL")
            return
